...
class MappingsViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val unseenToastMessages = MutableLiveData<MutableList<String>>()
    ...
    private fun syncMapping(mapping: Mapping){
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            phoneFiles.forEach { file -> run{
                if(!fileWriter.fileExistsInShare(sharePath, file.name)){
                    try{
                        ...
                        addToastMessage("${file.name} failed to add")
            ...
            if(filesAdded > 0){
                addToastMessage("$filesAdded files added")
            }
    ...
    private fun addToastMessage(message:String){
        val list = unseenToastMessages.value ?: mutableListOf<String>()
        list.add(message)
        unseenToastMessages.postValue(list)
    }
